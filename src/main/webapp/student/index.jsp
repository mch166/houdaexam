	...
	function daojishi() {
		if (counttime >= 0) {
			...
			if (mis >= 60) {
				...
				document.getElementById("mss").innerHTML = hour + "小时" + mis
						+ "分" + ms + "秒";
...
<body>
	...
	<div class="container">
				...
				<ul class="headmenu">
					...
					<li class="right">
						<div class="userloggedinfo">
							...
							<div class="userinfo">
								<h5>${studentInfo.studentName}<small>you are
										welcome</small>
								</h5>
								<ul>
		...
		<div class="examed_uc maincontentinner">
			<h3 style="text-align: center;">厦门理工学院考试</h3>
			<br>
			<div class="exam_time">
				...
				<form action="submitExam.action" method="post">
					<s:iterator value="#request.subjects" var="subject" status="sta">
						<div class="question">
							...
							<p>${sta.index + 1}、${subject.stTitle}（）&nbsp;(<span
									class="D_score">20.00</span>分)
							</p>
							<ul>
								<li><input type="radio" name="stAnswer${sta.index}"
									value="A" checked="checked" /> A．${subject.stOptionA}</li>

								<li><input type="radio" name="stAnswer${sta.index}"
									value="B" />B．${subject.stOptionB}</li>

								<li><input type="radio" name="stAnswer${sta.index}"
									value="C" /> C．${subject.stOptionC}</li>

								<li><input type="radio" name="stAnswer${sta.index}"
									value="D" /> D．${subject.stOptionD}</li>
							</ul>
						</div>
					</s:iterator>
					<br>
